...
para que haja um abandono de uso dos combustíveis fósseis, e essa energia deve ser armazenada
em dispositivos como baterias recarregáveis baratas e seguras. Pela mesma razão, o mercado
...
cátodos. Os requerimentos básicos tanto para eletrólitos quando para revestimento de cátodos
...
impedância eletroquímica (EIS, do inglês \textit{Electrochemical
Impedance Spectroscopy}) usando um circuito equivalente como modelo \cite{sinfronio2018effect,santos2018chitosan}, que
...
\section{Espectro de Impedância Eletroquímica}

A espectroscopia de impedância eletroquímica é uma técnica utilizada para estudar sistemas eletroquímicos em
equilíbrio ou estado estacionário. Ela consiste na aplicação de um sinal senoidal (diferença de
...
aplicada. Ademais, pode ser usada para analisar processos eletroquímicos em sistemas,
principalmente em interfaces eletrodo-eletrólito. A EIS é utilizada em várias áreas da ciência e
engenharia. E suas principais aplicações são em corrosão, em estudo de caracterização de
eletrodos eletroquímicos, incluindo condutividade eletrônica, monitoramento de reações
eletroquímicas, biossensores eletroquímicos e entre outras inúmeras aplicações \cite{lazanas2023electrochemical}.

Dessa forma, ao realizar a transição de um circuito em corrente continua (DC) para um circuito
em corrente alternada (AC), é necessário expandir a noção de resistência como um parâmetro
exclusivo dos resistores. Além dos resistores (R), os capacitores (C) e indutores (L) também oferecem
resistência ao fluxo de corrente alternada, denominada de resistência reativa ou reatância. Sendo assim, a principal diferença entre resistência e reatância, em termos de dissipação de energia, é que em
uma resistência a energia é dissipada apenas na forma de calor, enquanto em uma reatância a
energia é periodicamente armazenada em campos elétricos e magnéticos, sem perdas de calor.
A combinação de resistências é reatância é conhecida como impedância (Z). A impedância é
...
\begin{equation}
    ...
    \label{eq:16}
\end{equation}
Assim, com base na relação \eqref{eq:14}, podemos observar que a parte real da impedância permanece constante independentemente da frequência.
...
Uma corrente constante não pode fluir através de um capacitor, mas a carga elétrica pode se acumular nela, além disso essa carga é diferente para cada tensão aplicada. Vimos a relação fundamental entre a carga e tensão na equação \eqref{eq:3} e, também, a reatância capacitiva na
...
\begin{equation}
	\omega_{Z''\_max} = \frac{1}{\tau} = \frac{1}{R_1 C_1}.
	\label{eq:r1c1}
\end{equation}
...
A quantidade de um modelo de classificação é medida por meio de
...
exemplo de como a árvore de decisão funciona, utilizando o conjunto de dados de íris. O termo
árvore de decisão se justifica pelo fato de ela começar com um nó raiz, semelhante ao tronco
...
a primeira pergunta feita é: se o comprimento da pétala da flor é inferior a 2.45 cm? Se a resposta
for afirmativa, prosseguimos para o nó filho esquerdo na profundidade 1, à esquerda. Este nó é
uma folha, o que significa que não há mais perguntas a serem feitas. A árvore de decisão prevê
que a flor é uma íris setosa.

Por outro lado, se o comprimento da pétala for superior a 2.45 cm,
...
Dados um conjunto de vetores de treinamento $x_i \in \mathbb{R}^n$, onde $i = 1, \ldots, l$ é um vetor de rótulos $y \in \mathbb{R}^l$, uma árvore de decisão particiona de forma recursiva o espaço de características para agrupar amostras com rótulos semelhantes em conjunto. Cada nó da árvore, representado por $Q_m$, contém $n_m$ amostras. Para cada possível candidato se divide $\theta = (j, t_m)$, que consiste em um recurso $j$ e o limiar $t_m$, os dados são divididos em dois subconjuntos $Q_m^{esquerdo}(\theta)$ e a $Q_m^{direito}(\theta)$.
...
\begin{equation}
    ...
    \label{eq:34}
\end{equation}
A qualidade de uma candidata dividida no nó $m$ é calculada usando uma função de impureza ou função de perda $H$. A escolha da função depende da tarefa em questão, que pode ser classificação ou regressão. A métrica de qualidade $G(Q_m, \theta)$ é definida como:
...
Nesse contexto, existe um parâmetro chamado critério de classificação, onde se um alvo for um resultado de classificação pode assumir valores $0, 1, \ldots, k-1$, para o nó $m$, deixar

\begin{equation}
    p_{mk} = \frac{1}{n_m} \sum_{y \in Q_m} I(y = k)
    \label{eq:37}
\end{equation}
ser um alvo de observações de classe $k$ no nó $m$. Se $m$ é um nó terminal, o método estima\textit{predict\_proba} a probabilidade correspondente, pois esta região é definida como $p_{mk}$. As medidas comuns de impureza são as respectivamente Gini, dada pela equação \eqref{eq:38}, e Entropria, equação \eqref{eq:39}:
\begin{equation}
...
\begin{equation}
    ...
    \label{eq:39}
\end{equation}
Vale ressaltar que o conceito de entropia se originou como uma grandeza termodinâmica, na qual mede o grau de liberdade como uma medida de desordem molecular.
...
A RF é uma combinação de múltiplas árvores de decisão. Ademais, árvore de decisão profundas podem
...
É uma técnica de amostragem na qual várias amostras aleatórias são criadas a partir do
conjunto de dados de treinamento. O objetivo é gerar novos dados para prever futuros dados
reais e aprimorar o desempenho do modelo. Essa abordagem constrói o novo aprendiz de forma
sequencial, onde cada classificador é treinado com base nos dados, considerando o desempenho
dos classificadores anteriores. Após cada etapa de treinamento, os pesos são redistribuídos. A
...
\subsubsection{Light gradiente boosting}
\textit{Light Gradiente Boosting Machine} (LightGBM) é um algoritmo de aprendizado de
máquina desenvolvida pela Microsoft e disponibilizado como código aberto \textit{(open-source)}. Ele
se baseia no modelo de Gradiente Boosting (GB), mas é mais rápido e eficiente que o GB
...
\subsubsection{\textit{Extrement gradient boosting} (XGBoost)}
O \textit{}{Extreme Gradient Boosting} (XGBoost) é um algoritmo de aprendizado de
máquina baseado no método de \textit{Gradiente Boosting}, e amplamente utilizado para resolver
...
• Verdade negativa (VN): quando o modelo prevê corretamente a classe
negativa;
...
logarítmicas, iniciando com 30 décadas. Os parâmetros de entrada empregados na geração
